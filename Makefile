LANGUAGE_NAME := tree-sitter-fey
HOMEPAGE_URL := https://github.com/Loreite/tree-sitter-fey
VERSION := 0.1.0
DESCRIPTION := A parser for the configurable Fey markup language
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/fey '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/fey
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/fey
